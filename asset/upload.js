    ...
    window.Upload = function() {
        ...
        function bind(idCfg) {
            ...
            if (ele) {
                ...
                if (effect === 'change') invoker = ele;
                else invoker = document.getElementById(idCfg.btnId);

                if (invoker) on(invoker, effect, function() {
                    if (ele.value === '') return;
                    ...
                    if (tempV.result) {
                        ...
                        if (idCfg.beforeSubmit) idCfg.beforeSubmit();
                        ...
                        if (outId && !/outId=(\d+)(?=\&|$)/.test(fm.action)) {
        ...
        function fetchUrl() {
            ...
            if (fEle) success = fEle.innerHTML;
            else success = f.body.innerHTML;
            ...
            if (success !== '') {
                ...
                pid = -1;
                for (var i = 0; i < eleArr.length; ++i) {
                    if (eleArr[i].id !== indId) eleArr[i].disabled = false;
                }
                ...
                if (!jsobj || !jsobj.model) {
                    ...
                    errorMsg(indId, '上传文件失败！');
                    return;
                }
                jsobj = jsobj.model;
                var uurl = '';
                //判断是否已经session失效，若是则重定向
                if (jsobj.sessionTimeout) location.reload();
                else if (jsobj.formError) {
                    var html = [];
                    var err = jsobj.formError;
                    for (var attr in err) html.push(err[attr]);
                    if (jsobj.lbsUpload) {
                    ...
                    if (jsobj.file) uurl = jsobj.file;
                    errorMsg(indId, '');
                    if (jsobj.locations) { //lbs特殊情况
                        if (callback) callback(uurl, jsobj.locations);
                        return;
                    }
                }
                ...
                if (callback && uurl !== '') callback(uurl, jsobj);
            }
        }
        ...
        function showLink(id, url) {
            ...
            if (showLink) {
                ...
                showLink.style.display = url === '' ? 'none' : styleMap[indId];
            }
        }
        ...
        return {
            ...
            reg: function(cfg) {
                var ids = cfg.ids || [];
                if (formId === '') formId = cfg.formId || 'myForm';
                var fm = document.getElementById(formId);
                if (!fm) return;
                if (act === '') act = cfg.action || fm.action;
                ...
                if (ids.length === 0) ids = [cfg.info];
                for (var i = 0; i < ids.length; ++i) {
                    bind(ids[i]);
                    if (!(ids[i].hide)) hide(ids[i]);
                }
            }
        };
    };
