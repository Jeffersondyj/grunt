...
Sync.prototype.syncSite = function () {
    ...
    me.MODULE_LIST = [];
    me.privateModuleFiles = [];
    me.commonModuleFiles = [];
    ...
    for (var m in module) {
        if (module.hasOwnProperty(m)) {
            if (module[m].private) {
                ...
                me.privateModuleFiles.push(m + '.js');
            } else {
                console.log('load site common module', m);
                me.commonModuleFiles.push(m + '.js');
                me.MODULE_LIST.push(m);
            }
        }
    }
    me.propCache.push(me.site + 'sitecommonmodule.files=' + me.commonModuleFiles.join(' '));
    me.propCache.push(me.site + 'siteprivatemodule.files=' + me.privateModuleFiles.join(' '));
    //pages
...
Sync.prototype.syncPage = function (page) {
    ...
    me.jsFiles.length = 0;
    me.cssFiles.length = 0;
    me.htmlFiles.length = 0;
    me.privateModuleFiles.length = 0;
    me.commonModuleFiles.length = 0;
    ...
    me.propCache.push(me.render(me.PROP_TPL, {
        'commonmodule': me.commonModuleFiles.join(' '),
        'privatemodule': me.privateModuleFiles.join(' '),
        'js': me.jsFiles.join(' '),
...
Sync.prototype.syncAction = function (page, module, action) {
    ...
    var ACTION_DATA_DIR = path.join(me.DATA_DIR, page);
    var ACTION_DATA_FILE = path.join(me.DATA_DIR, page, action + '.js');
    ...
    me.dataFiles.push(fm('%s/%s.js', page, action));
};
...
Sync.prototype.syncMockData = function (page, module, dataNames) {
    ...
    for (var name in dataNames) {
        console.log('Sync', me.site, page, name, 'mockData');
        ...
        me.dataFiles.push(fm('%s/%s.js', page, name));
    }
};
...
Sync.prototype.render = function (tplPath, data) {
    ...
    data.author = data.author || 'Yijun Deng<dengyijun@baidu.com>';
